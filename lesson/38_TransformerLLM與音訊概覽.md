...
本課先集中理解：模型看到的文字可以怎樣被分成 token，已讀取的 context 又怎樣影響下一個 token。

## 新概念

> **本課不會訓練 Transformer。** 我們只理解 LLM 處理文字時最基本的輸入、context 和輸出概念；音訊會放在後面的延伸部分。
...
![文字由 token 組成；模型查看 context，再逐步產生文字，並延伸比較音訊的取樣波形](/images/lesson-38-transformer-llm-audio.svg)
...
用自己的話說明：模型根據 context 預測下一個 token，為甚麼不等於它已查證這個 token 的內容？
